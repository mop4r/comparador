...
from PyQt5.QtWidgets import (
    ...
    QDialog, QProgressBar, QLabel, QTextBrowser, QMessageBox, QInputDialog, QFileDialog
)
...
class ResultDialog(QDialog):
    def __init__(self, db1, db2, table_name, result_db1, result_db2):
        ...
        copy_button_db1 = QPushButton("Copiar")
        copy_button_db1.clicked.connect(lambda: self.copy_result(result_db1))

        save_button_db1 = QPushButton("Salvar em arquivo .txt")
        save_button_db1.clicked.connect(lambda: self.save_to_txt(result_db1, db1, self.table_name))
        ...
        layout.addWidget(save_button_db1)
        layout.addWidget(label_db2)
        layout.addWidget(text_browser_db2)

        copy_button_db2 = QPushButton("Copiar")
        copy_button_db2.clicked.connect(lambda: self.copy_result(result_db2))
        layout.addWidget(copy_button_db2)

        save_button_db2 = QPushButton("Salvar em arquivo .txt")
        save_button_db2.clicked.connect(lambda: self.save_to_txt(result_db2, db2, self.table_name))
        layout.addWidget(save_button_db2)

        self.setLayout(layout)

    def save_to_txt(self, data, db, table_name):
        print("Entrando no método save_to_txt")
        file_path, _ = QFileDialog.getSaveFileName(self, "Salvar em arquivo .txt", "", "Arquivo de Texto (*.txt)")
        print("Caminho do arquivo selecionado:", file_path)
        if file_path:
            with open(file_path, "w") as file:
                file.write(f"Registros presentes somente no banco {db} na tabela {table_name}:\n")
                file.writelines([f"{str(row)};\n" for row in data])
            QMessageBox.information(self, "Arquivo Salvo", "Os resultados foram salvos em arquivo .txt com sucesso.")

    def copy_result(self, data):
